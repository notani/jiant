...
class JSAATask(SingleClassificationTask):
    ...
    def __init__(self, path, max_seq_len, name, **kw):
        ...
        assert word_segmentation in ['ipadic', 'jumanpp', 'unidic', 'spm', 'sudachi']
...
@register_task('jsar', rel_path='ja/SAR-')
class JSARTask(SingleClassificationTask):
    ''' Task class for Japanese Sentiment Analysis dataset from Amazon. '''

    def __init__(self, path, max_seq_len, name, **kw):
        ''' '''
        n_classes = kw.pop('jsar_class')
        assert n_classes in [2, 5]  # Binary or five-class
        word_segmentation = kw.pop('word_segmentation')
        assert word_segmentation in ['ipadic', 'jumanpp', 'unidic', 'spm', 'sudachi']
        spm_vocabsize = kw.pop('spm_vocabsize')  # {8000, 16000, 32000, None}
        assert spm_vocabsize in ['8000', '16000', '32000', 'none']
        path = os.path.join(path + str(n_classes), word_segmentation)
        if spm_vocabsize in ['8000', '16000', '32000']:
            path = os.path.join(path, spm_vocabsize)
        super(JSARTask, self).__init__(name, n_classes=n_classes, **kw)
        self.load_data(path, max_seq_len)
        self.sentences = self.train_data_text[0] + self.val_data_text[0]
    ...
    def load_data(self, path, max_seq_len):
        ...
        self.val_data_text = val_data
        self.test_data_text = te_data
        log.info("\tFinished loading jSAR data.")
